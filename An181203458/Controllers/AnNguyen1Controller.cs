...
{
    public class AnNguyen1Controller : Controller
    {
        ...
        // GET: Ajax hiển thị theo Search tên hàng
        public ActionResult RenderProductByName(String id)
        {
            var listHangHoa = db.HangHoas.Where(s => s.Gia >= 100).ToList();
            if (id != null)
            {
                listHangHoa = db.HangHoas.Where(s => s.TenHang.Contains(id)).ToList();
            }
            var listHangHoa2 = listHangHoa.OrderBy(x => x.MaHang); //Sắp xếp theo mã hàng
            return PartialView("_An_MainContent2", listHangHoa2);
        }
